...
For example let's consider s a gyroscope (a sensor which output is the angular speed). Gyroscopes output is noisy and its bias is described by a Brownian motion (it changes randomly in time). If we can mitigate the noise with a low pass filter, how can we estimate the gyroscope bias?

Another problem that we want to solve is related to the best operative conditions of a sensor. 

Let's consider a spinning satellite. A possible way to estimate its orientation is to use a star sensor (a sensor that recognize the starts in the sky and, since stars position is constant in time, it outputs the satellite orientation). Since star sensors might have a slow refresh rate, they usually have a bad estimate of the angular velocity (useful for control purposes). Moreover, a slow refresh rate means a "mostly coarse" orientation estimate (the orientation will be accurate near the sensor update). 
